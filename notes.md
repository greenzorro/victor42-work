...
- 两个背景模块都支持幂等 `prepare()`，只生成或复用可跨主题切换使用的模块资源，不设置 `wantRun`，也不启动 RAF。树影预生成固定枝叶纹理；星系记录当前视口指标并生成密度纹理与粒子。非当前主题的全视口显示 Canvas 等到实际 `start()` 时才分配
- `loadProducts()` 结束后，`main.js` 先等待当前背景进入稳定运行，再通过 `requestIdleCallback` 预热非当前主题，不支持时使用 1.2 s 定时器。主题切换使用双层 `requestAnimationFrame`，先绘制页面颜色与按钮图标，再启停背景动画；视口变化或页面重新可见后会重新安排预热
...
- **枝叶生成**：树影由 `prepare()` / `start()` 按需生成，并在模块生命周期内复用固定纹理；固定随机种子确保远、中、近三层枝叶轮廓稳定。疏密由角点距离、外弧距离、低频斑块噪声和三层共享的高频微型漏光场共同决定；微型漏光场会降低局部叶片数量，形成小型稀疏区。每层包含 5 根可见主枝，分别对应 5 个独立扇形运动区；每根主枝的后 20% 与末端都有随枝条同步运动的专属叶团，摇曳时不会从树冠中露出光秃枝梢
...
- **动态**：所有运动区共享由长、中、短周期和高频扰动四个非整数倍波形叠加成的风力，默认风力倍率为 1.35。风力以 0–1.60 s 的时差到达不同枝组和景深层，使形变顶点与回弹起点错开，同时保持总体风向一致；各区域再以略有差异的刚度和近临界阻尼响应。首次进入可动画态时，模拟时钟若仍为 0 则按种子跳到约 2–7 s，并将各组弹簧位移与速度直接对齐到当时风场（含时差与相位），首帧即处于错相摆动；风力没有短周期重复，减弱后枝条回到中性位置，默认 24 FPS
- **资源与兼容性**：15 张固定枝组纹理不依赖视口尺寸，窗口 resize 只调整显示 Canvas 与合成蒙版，并按当前模拟时间把枝条弹簧重新对齐到风场，继续复用纹理。同一视口内多次调用 `start()` / `stop()` 也会复用资源；`destroy()` 会停止动画、解除监听器并清除 resize 定时器。树影不依赖外部图片，要求浏览器支持 Canvas 2D `filter: blur()`
...
- **渲染管线**：
  ...
  3. 资源就绪时烘焙静态天空层（空间渐变、场星、晕星）与场星/晕星光晕 sprite
  4. 每帧：blit 天空层，再按深度穿插盘面 Z 切片、实时核球 bloom 与可见盘面粒子
  5. 盘面粒子每数帧完整重投影并按深度排序；中间帧只刷新已可见星的屏幕坐标
- **动态与视觉**：盘面逆时针旋转，拖曳臂使用 θ = -ln(r)/b。近侧尘埃切片覆盖核球，旋臂间隙透出背景恒星以形成视差深度；尘埃带延伸至核心边缘，核球以暖黄、粉橘至白炽渐变过渡，内旋臂为低饱和紫调，外缘为窄带冰蓝色。盘面以 5 层 Z 切片叠加厚度感，切片 `drawImage` 使用 `medium` 平滑；核球 bloom 每帧实时绘制并与盘面层 `lighter` 混合，核心星点使用与粒子同色的径向光晕，中央呈现混光质感
- **资源与调度**：背景星场以 1600 颗星点为基准，并按视口面积缩放数量；窄屏（`<640px`）额外下调粒子与场星规模、将 DPR 上限压到 1.25、目标帧率为 24。盘面星点集中于旋臂与核心区域。整数格点噪声结果会被缓存；粒子笛卡尔坐标、颜色前缀和视图不变量预先计算；静态天空与场星/晕星光晕 sprite 在资源提交时烘焙，每帧对天空层只做一次 `drawImage`。可见粒子每 3 帧完整重投影与深度排序一次，中间帧只更新已可见星点的屏幕坐标。小幅 resize（视口面积变化 ≤15%）只重同步星点坐标并重建天空缓存，不重算星云纹理。同一视口内多次调用 `start()` / `stop()` 会复用纹理与粒子。`prepareAsync()` 把星云像素生成拆成每批最多 2 行、6 ms 的闲时任务；若生成尚未完成便启动星系，则提高为每批最多 4 行、9 ms，且每批结束后让出主线程。`destroy()` 会停止动画、解除监听器、清除 resize 定时器，并使进行中的资源生成失效
...
| 参数 | 默认值 | 作用 |
|------|--------|------|
| `targetFps` | `30` | 桌面动画目标帧率；窄屏运行时为 24 |
| `rotationPeriodSec` | `300` | 盘面完整旋转一周的秒数 |
| `dprCap` | `1.5` | 桌面 Canvas 设备像素比上限；窄屏运行时上限为 1.25 |
