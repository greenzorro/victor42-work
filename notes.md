...
- **资源与兼容性**：15 张固定枝组纹理不依赖视口尺寸，窗口 resize 只调整显示 Canvas 与合成蒙版、复位枝条运动状态，并继续复用纹理。同一视口内多次调用 `start()` / `stop()` 也会复用资源；`destroy()` 会停止动画、解除监听器并清除 resize 定时器。树影不依赖外部图片，要求浏览器支持 Canvas 2D `filter: blur()`
